...
function lxins_main()
{
	...
	if ($kloxo_already_exist) {
	    print("Kloxo already exists. Select 'y' if want re-install or 'n' for replace core\n");
	    print("- Select 'n' if your Kloxo running well\n");
...
function kloxo_replace_core()
{
	...
	foreach ($a as &$v) {
	    system("mv -f {$kloxo_path}/{$v}");
	}

	system("cp -rf ../kloxo-current.zip {$kloxo_path}");

	system("cd {$kloxo_path}; unzip -oq kloxo-current.zip");

	system("sh /script/cleanup");
	system("sh /script/fixdns");
	system("sh /script/fixweb");
	system("sh /script/fixphp");
	system("sh /script/fixwebmail");
}
...
// system("cp -rf kloxo-custom.repo /etc/yum.repos.d/kloxo-custom.repo");
}
