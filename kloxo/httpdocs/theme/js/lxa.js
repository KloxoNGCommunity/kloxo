	...
	function updateallWarning() {
		if (confirm("UpdateAll will impress the parameters of the above form to EVERY object in the drop down list at the top. " + 
                "Do you want to continue? If unsure, press cancel, and use simple update")) {
			if (confirm("Are you really sure?\n\n\n\n\nEvery object in the drop down list at the top will be updated with the values in the above form. " + 
	...
	function selectFolder(obj, rootFolder, url) {
		selFolderObj = obj;
		windowFolderSel = window.open(url, 'FolderSel', 'toolbar=no,location=no,directories=no,' + 
                'status=no,menubar=no,scrollbars=yes,resizable=no,titlebar=no,width=500,height=400,top=' + 
                parseInt((window.screen.height - 185) / 2) + ',left=' + parseInt((window.screen.width - 210) / 2));
	...
	function swapOptions(element, index1, index2) {
		...
		element.options[index1] = new Option(element.options[index2].text, element.options[index2].value, 
                element.options[index2].defaultSelected, element.options[index2].selected);
	...
	function statechange(element, ename, frmname, status) {
		...
		if (status == 1) {
			classtype = 'textdisable';
		} else {
			classtype = 'textenable';
		}
							...
							if (status == 0) {
								...
								if (frmname.elements[i].type == 'select-one') {
									...
									if (frmname.elements[i].className == 'ckbox1') {
										...
										(frmname.elements[i].className == 'ckbox2') frmname.elements[i].className = 'dckbox2';
									} else {
										frmname.elements[i].className = classtype;
									}
								}
	...
	function changerights(frmname, element, mode, val) {
		if (mode == 'user') {
			txt = frmname.user;
			ckall = frmname.userall;
		}

		if (mode == 'group') {
			txt = frmname.group;
			ckall = frmname.groupall;
		}

		if (mode == 'other') {
			txt = frmname.other;
			ckall = frmname.otherall;
		}
	...
	function UpdateClock() {
		...
		document.theClock.theTime.value = "" + hours + ":" + minutes + ":" + tDate.getSeconds();
