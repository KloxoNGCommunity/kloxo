...
function PrepareRoundCubeDb($nolog = null)
{
	...
	system("mysql -f -u root $pstring roundcubemail < /home/kloxo/httpd/webmail/roundcube/SQL/mysql.initial.sql " .
			">/dev/null 2>&1");
...
function PrepareHordeDb($nolog = null)
{
	...
	system("mysql -f -u root $pstring < /home/kloxo/httpd/webmail/horde/scripts/sql/groupware.mysql.sql " .
			">/dev/null 2>&1");
...
function installWithVersion($path, $file, $ver = null, $nolog = null)
{
	...
	if (lxfile_exists("/var/cache/kloxo/kloxo-install-firsttime.flg")) {
		...
		if (lxfile_exists("/var/cache/kloxo/$file$locver.tar.gz")) {
			...
			system("cd /var/cache/kloxo/ ; rm -f $file*.tar.gz ; " .
 ...
 */
	if (lxfile_exists("/var/cache/kloxo/kloxo-install-firsttime.flg")) {
		...
		if (lxfile_exists("/var/cache/kloxo/$prgm-thirdparty.$locver.zip")) {
			...
			system("cd /var/cache/kloxo/ ; rm -f $prgm-thirdparty.*.zip ; " .
