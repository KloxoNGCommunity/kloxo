...
function get_ifconfig($key = null)
{
	...
	// MR - 'ifconfig' method - mod from http://www.plugged.in/linux/getting-network-information-in-bash-scripts.html
	exec("ifconfig | grep -w inet |grep -v 127.0.0.1| awk '{print $2}' | cut -d \":\" -f 2", $out);
	$list['ipaddr']  = $out[0];
	exec("ip addr show |grep -w inet |grep -v 127.0.0.1|awk '{ print $2}'| cut -d \"/\" -f 2", $out);
//	$list['ipprefix']   =  $out[0];
//	exec("ifconfig | grep -w inet |grep -v 127.0.0.1| awk '{print $3}' | cut -d \":\" -f 2", $out);
//	$list['broadcast']  =  $out[0];
//	exec("ifconfig | grep -w inet |grep -v 127.0.0.1| awk '{print $4}' | cut -d \":\" -f 2", $out);
	$list['netmask']    =  $out[0];
	exec("ifconfig | grep HWaddr | cut -d \" \" -f 1", $out);
	$list['devname'] =  $out[0];
	exec("ifconfig | grep -w inet6 | grep -v ::1| awk '{ print $3 }' | cut -d \"/\" -f 1", $out);
	$list['ip6addr'] =  $out[0];
//	exec("ifconfig | grep -w inet6 | grep -v ::1| awk '{ print $3 }' | cut -d \"/\" -f 2", $out);
//	$list['ip6prefix']  =  $out[0];
//	exec("ifconfig | grep HWaddr | awk '{ print $5 }'");
//	$list['macaddress'] =  $out[0];
/*
	// MR - 'ip' method - mod from http://www.plugged.in/linux/getting-network-information-in-bash-scripts.html
	exec("ip addr show |grep -w inet |grep -v 127.0.0.1|awk '{ print $2}'| cut -d \"/\" -f 1", $out);
	$list['ipaddress']  =  $out[0];
	exec("ip addr show |grep -w inet |grep -v 127.0.0.1|awk '{ print $2}'| cut -d \"/\" -f 2", $out);
	$list['ipprefix']   =  $out[0];
	exec("ip addr show |grep -w inet |grep -v 127.0.0.1|awk '{ print $4}'", $out);
	$list['broadcast']  =  $out[0];
	exec("ifconfig | grep -w inet |grep -v 127.0.0.1| awk '{print $4}' | cut -d \":\" -f 2", $out);
	$list['netmask']    =  $out[0];
	exec("ip addr show |grep -w inet |grep -v 127.0.0.1|awk '{ print $7}'", $out);
	$list['devicename'] =  $out[0];
	exec("ip addr show |grep -w inet6 |grep -v ::1|awk '{ print $2}'| cut -d \"/\" -f 1", $out);
	$list['ip6address'] =  $out[0];
	exec("ip addr show |grep -w inet6 |grep -v ::1|awk '{ print $2}'| cut -d \"/\" -f 2", $out);
	$list['ip6prefix']  =  $out[0];
	exec("ip addr show | grep -w ether | awk '{ print $2 }'", $out);
	$list['macaddress'] =  $out[0];
*/
	if ($key) {
		return $list[$key];
...
function get_ifcfg($file, $key = null)
{
